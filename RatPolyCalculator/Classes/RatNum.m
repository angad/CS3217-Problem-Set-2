...
+(RatNum*)initNaN{
  ...
  [nan autorelease];
  // we autorelease the allocated object, so that the caller
  // does not have the burden of releasing it
  return nan;
}
...
+(RatNum*)initZERO{
  ...
  RatNum *zero = [[RatNum alloc] initWithInteger:0];
  [zero autorelease];
  return zero;
}
...
-(RatNum*)negate{
  ...
  return [[[RatNum alloc] initWithNumer:-self.numer Denom:self.denom] autorelease];
}
...
-(RatNum*)add:(RatNum*)arg{
  ...
  return [[[RatNum alloc] initWithNumer:new_numer Denom:new_denom] autorelease];
}
...
-(RatNum*)mul:(RatNum*)arg{
  ...
  return [[[RatNum alloc] initWithNumer:self.numer*arg.numer Denom:self.denom*arg.denom] autorelease];
}
...
-(RatNum*)div:(RatNum*)arg{
  ...
  if ([arg isNaN]) {
    ...
    return [[[RatNum alloc] initWithNumer:self.numer*arg.denom Denom:self.denom*arg.numer] autorelease];
  }
}
  ...
  // EFFECTS: return a RatNum represented by "str"
	...
	if ([str isEqual:@"NaN"]) {
		...
		if ([tokens count] == 1) {
			...
			return [[[RatNum alloc] initWithInteger:arg] autorelease];
		} else {
